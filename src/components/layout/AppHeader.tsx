...
export function AppHeader() {
  ...
  const quickActions = [
    {
      title: t('header.quickBookCourt'),
      icon: Target,
      action: () => setIsAppointmentModalOpen(true),
      type: 'court'
    },
    {
      title: t('header.quickBookSwimming'),
      icon: Waves,
      action: () => setIsAppointmentModalOpen(true),
      type: 'swimming'
    },
          ...
          <div className={`flex items-center space-x-2 ${isRTL ? 'space-x-reverse' : ''}`}>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className={`flex items-center space-x-2 ${isRTL ? 'space-x-reverse' : ''} hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl p-2`}>
                  <div className="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-600 rounded-full flex items-center justify-center">
                    <User className="h-4 w-4 text-white" />
                  </div>
                  <div className={`hidden md:block ${isRTL ? 'text-right' : 'text-left'}`}>
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{t('header.userName')}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">{t('header.userRole')}</p>
                  </div>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem className={`flex items-center space-x-2 ${isRTL ? 'space-x-reverse' : ''} text-red-600 dark:text-red-400`}>
